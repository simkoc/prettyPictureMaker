...
object PrettyPictureMaker extends LogSupport {
  ...
  private val parser: Parser = Parser("PrettyPictureMaker",
                                      "using raw input creates nice outputs")
    .addSubparser(
      Parser("dummy", "use this to run the current flavor of test")
    ...
    .addSubparser(Parser("barPlot", "create bar plots")
      .addPositional(
        "csv",
        "input file containing the raw data (ledeco analysis trackerStatistics)")
      .addPositional("orientation",
                     "the orientation of the bar {horizontal,vertical}")
      .addPositional("out", "the path to the file that shall be created (no ending, a .tex and .svg will be created)")
      .addOptional("sort",
                   "s",
                   "sort",
                   None,
                   s"how (if) to sort the values {asc,desc}")
  ...
  private def barPlotMain(pargv: ParsingResult): Unit = {
    ...
    val sorting = pargv.get[Option[String]]("sort") match {
      ...
      case None         => None
      case value =>
        throw new RuntimeException(s"the sorting value $value is not supported")
    }
