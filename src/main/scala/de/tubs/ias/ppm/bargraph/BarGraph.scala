...
class BarGraph(outSvg : String, axis : Axis, plots : Seq[Plot]) extends TikzSVG(outSvg) {
...
object BarGraph extends LogSupport {
  ...
  def fromCsv(csvs : Seq[String], out : String, orientation: BarOrientation, offset : Int = 0, steps : Int = 1, sorting: Option[SortingDirection] = None): BarGraph = {
    val plots = csvs.map {
      csv =>
      ...
      try {
        var lineCounter = 0
        val coordinates : Seq[Coordinate] = csvSource.getLines().map {
          line =>
            lineCounter += 1
            line.split(",").toList match {
              ...
              case _ =>
                throw new RuntimeException(s"line $lineCounter has bad format : ${line.mkString(",")} (expected 2 values only)")
            }
        }.toSeq
        ...
        val sorted = sorting match {
          ...
          case value => throw new RuntimeException(s"this must never happen - the fuck is sorting $value")
        }
